...
if uploaded_file is not None:
    # Save uploaded PDF to temp file for loader compatibility
    ...
    query = st.text_input("Ask a question related to the document", value=selected_question if selected_question else "")
    ...
    if query:
        with st.spinner("Searching for answer..."):
            # Find similar docs (not used explicitly but part of get_answer)
            relevant_docs = get_similar_docs(index, query)

            # Run QA chain on retrieved documents
            result = chain.run(input_documents=relevant_docs, question=query, language=language)
